...
const buttons = document.querySelectorAll('submenuRowBtn');
buttons.forEach(btn => {
    btn.addEventListener('mouseenter', (event) => {
        event.target.parentNode.qetElementsByClassName('clickSubmenu')[0].style.display = 'block'
    }, false);
    btn.addEventListener('mouseleave', (event) => {
        event.target.parentNode.qetElementsByClassName('clickSubmenu')[0].style.display = 'none'
    }, false);
});